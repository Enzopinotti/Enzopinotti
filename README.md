...
Meow Matrix's **provider-neutral engineering carrier is complete through B7**; what remains there is deliberately external production work — real registry/provider credentials, production data/storage/SMTP/DNS/TLS, deploy-by-digest and final public qualification — rather than more simulated infrastructure. The next two historical lanes — **`App_Agenda_Medico`** and **`Web_de_vinos`** — are now complete with their original provenance preserved and permanent CI in place. The next general maintenance lane is **`Ecommerce_NestJS`**, as tracked in **[Repository portfolio roadmap — 2026](./docs/repository-portfolio-roadmap-2026.md)**.
